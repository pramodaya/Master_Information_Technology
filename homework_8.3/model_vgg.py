...
def get_out_size(in_size, padding, kernel_size, stride):
    result = (in_size + 2 * padding - kernel_size)
    return result
...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.encoder = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            Conv2d(in_channels=9, out_channels=16, kernel_size=3, stride=1, padding=1),
        )
        w_0 = dataset_full.input_size
        w_1 = get_out_size(in_size=w_0, kernel_size=4, stride=2, padding=1)
        w_2 = get_out_size(in_size=w_1, kernel_size=4, stride=2, padding=1)
        w_3 = get_out_size(in_size=w_2, kernel_size=3, stride=1, padding=1)
